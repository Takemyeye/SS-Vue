...
const saveOrUpdateUserOnServer = async (user) => {
  try {
    const response = await fetch('https://ss-vue-yf04.onrender.com/api/user', {
      method: 'POST',
      headers: {
...
const clearUserFromServer = async () => {
  ...
  try {
    await fetch('https://ss-vue-yf04.onrender.com/api/user', {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${state.token}`,
      },
...
const getUserFromCode = async (code) => {
  try {
    ...
    if (userResponse) {
      ...
      const tokenResponse = await fetch('https://ss-vue-yf04.onrender.com/auth/google/callback', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
